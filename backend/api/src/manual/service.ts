...
export class ManualMovementService {
  ...
  async confirmWithStatus(context: LedgerContext, input: unknown) {
    return this.database.asUser(context.userId, (c) =>
      this.confirmInTransaction(c, context, input),
    );
  }
  /** Caller owns the transaction and must propagate every failure. */
  async confirmInTransaction(c: PoolClient, context: LedgerContext, input: unknown) {
    ...
    if (a.state !== 'active' || cat.state !== 'active') throw new DomainError('REFERENCE_INACTIVE');
