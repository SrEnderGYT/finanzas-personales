...
export function instant(value: string): string {
  if (
    ...
    throw new DomainError('INVALID_INSTANT');
  civil(value.slice(0, 10));
  const parsed = new Date(value);
  if (!Number.isFinite(parsed.getTime()) || parsed.toISOString().length !== 24)
    throw new DomainError('INVALID_INSTANT');
...
export function financialDate(value: FinancialDate, clock: Clock): FinancialDate {
  ...
  if (occurredAt && localDate(new Date(occurredAt), timezone) !== businessDate)
    throw new DomainError('DATE_MISMATCH');
