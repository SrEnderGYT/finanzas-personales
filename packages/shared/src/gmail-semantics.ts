...
const CATEGORY_RULES: readonly [RegExp, AutomaticFinanceCategory][] = [
  [/\b(rappi|pedidosya|delivery|restaurant|restaurante|caf[eé]|starbucks|kfc|bembos|mcdonald|burger king|papa john|pizza hut)\b/i, 'Comida y delivery'],
  [/\b(wong|plaza vea|pvea|tottus|metro|vivanda|makro|vega|mass|supermercado)\b/i, 'Supermercado'],
  [/\b(uber|didi|yango|cabify|taxi|rides?|beat|inDrive|pasaje urbano|metropolitano)\b/i, 'Transporte'],
  [/\b(latam|jetsmart|sky airline|avianca|airbnb|booking|hotel|hostel|aerolinea|aerol[ií]nea|pasajes?|vuelo)\b/i, 'Viajes'],
  [/\b(chatgpt|openai|spotify|netflix|prime video|amazon prime|disney\+?|hbo|max\.com|youtube premium|icloud|google one|microsoft 365|canva|adobe|dropbox)\b/i, 'Suscripciones'],
  [/\b(luz del sur|enel|sedapal|claro|movistar|entel|win(?:\.pe)?|internet|telefon[ií]a|electricidad|agua)\b/i, 'Servicios'],
  [/\b(cinemark|cineplanet|steam|playstation|xbox|nintendo|hoyoverse|teatro|concierto|ticketmaster)\b/i, 'Entretenimiento'],
  ...
  [/\b(amazon|mercadolibre|mercado libre|falabella|ripley|oechsle|h&m|zara|ecomputer|izipay|izi\*)\b/i, 'Compras'],
];
...
export function isDashboardGmailMovement(candidate: GmailFinancialCandidate) {
  ...
  return candidate.kind === 'expense' || candidate.kind === 'card_charge' || candidate.kind === 'income';
}
