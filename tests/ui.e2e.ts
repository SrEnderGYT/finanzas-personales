...
test('desktop dashboard filters and DEMO form are functional', async ({ page }) => {
  const errors: string[] = [];
  page.on('pageerror', (e) => errors.push(e.message));
  await page.setViewportSize({ width: 1440, height: 1000 });
  await page.goto('/');
  await expect(page.getByRole('heading', { name: 'Tu resumen', exact: true })).toBeVisible();
  await expect(page.locator('.metric-value').nth(1)).toHaveText('S/ 386.00');
  await page.getByRole('button', { name: 'USD Dólares' }).click();
  await expect(page.locator('.metric-value').nth(1)).toHaveText('US$ 24.00');
  await page.getByRole('button', { name: 'PEN Soles' }).click();
  await page.getByLabel('Rango financiero').selectOption('Hoy');
  await expect(page.locator('.metric-value').nth(1)).toHaveText('S/ 186.50');
  ...
  await page.getByRole('button', { name: 'Movimiento demo' }).click();
  await page.getByLabel('Importe', { exact: true }).fill('12.50');
  await page.getByLabel('Comercio o concepto de muestra').fill('Compra sintética de prueba');
  await page.getByRole('button', { name: 'Guardar demo de sesión' }).click();
  await expect(page.locator('.metric-value').nth(1)).toHaveText('S/ 398.50');
...
test('visible dashboard meets automated WCAG AA checks in both themes', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
